...
# Downloading cluster indices
clust_centers_m = np.load('/indices/clust_centers.npy')
app = Flask(__name__)

# Configs where tf server is up
...
def get_data():
    ...
    if (content_type == 'application/json'):
        ...
        rest_results = requests.post(f'http://{host}:{port_rest}/v1/models/{model}:predict', json=data)
        if rest_results.ok:
            rest_results = rest_results.json()
        else:
            print('not ok')
            print(rest_results)
